...
export function updateResultScreen({
  ...
  document.querySelector(".result__totalRounds").textContent = totalRounds.toString();
  document.querySelector(".result__currentRound").textContent = currentRound.toString();
  
  
  const resultPlayer = document.querySelector(".result__player");

  resultPlayer.textContent = "spilari spilaði " + playAsText(player);

  const resultComputer = document.querySelector(".result__computer");

  resultComputer.textContent = "tölva setti út " + playAsText(computer);

  if (result === 1) {
    document.querySelector(".result__result").textContent = `Þú vannst!\n Staðan er ${playerWins} – ${computerWins}`;
  } else if (result === -1) {
    document.querySelector(".result__result").textContent = `Tölvan vann :(\n Staðan er ${playerWins} – ${computerWins}`;
  } else if (result === 0) {
    document.querySelector(".result__result").textContent = `Jafntefli!\n Staðan er ${playerWins} – ${computerWins}`;
  }
}
